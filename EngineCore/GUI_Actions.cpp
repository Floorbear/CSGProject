...
void WorkSpace_Actions::delete_selected_meshes(){
    ...
    for (CSGNode* selected_mesh : workspace->selected_meshes){
        task_multi->add_task(new TreeModifyTask<CSGNode>("Delete Mesh", [=](){
...
void WorkSpace_Actions::delete_selected_models(){
    ...
    for (Model* selected_model : workspace->selected_models){
        task_multi->add_task(new TreeModifyTask<Model>("Delete Model", selected_model->get_parent(), [=](){
...
void WorkSpace_Actions::add_mesh_new(CSGNode* parent_node, const Mesh& mesh, const Transform& transform){
    ...
    workspace->transaction_manager.add(new TreeModifyTask<CSGNode>("Add Mesh " + mesh.get_name(), parent_node, [=, this](){
...
void WorkSpace_Actions::add_model_new(const Mesh& mesh, const Transform& transform){
    ...
    workspace->transaction_manager.add(new TreeModifyTask<Model>("Add Model " + mesh.get_name(), workspace->root_model, [=, this](){
...
void WorkSpace_Actions::move_model_to_parent(Model* model){
    workspace->transaction_manager.add(new TreeModifyTask<Model>("Move Model To Parent", model->get_parent()->get_parent(), [=](){
...
void WorkSpace_Actions::create_boolean_opertation_of_selected_models(CSGNode::Type type){
    ...
    TransactionTask* task = new TreeModifyTask<Model>(std::string("Create ") + CSGNode::type_string_values[(int)type] + " Of Selected Models", workspace->root_model, [=, this](){
        return workspace->root_model->add_child(new_model);
    });

    for (Model* selected_model : workspace->selected_models){
        task = task->link(new TreeModifyTask<CSGNode>("Reparent Mesh", [=](){
    ...
    workspace->transaction_manager.add(task->link(new TreeModifyTask<Model>("Delete Empty Models", workspace->root_model, [=](){
